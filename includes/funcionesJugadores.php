...
function modificarAmonestados($id,$refjugador,$refequipo,$reffixture,$amarillas,$azul,$rojas,$jugo,$cancha,$arquero,$puntos,$mejor,$goles) {
	$sql = "update tbamonestados
	set
	refjugador 	= ".$refjugador.",
	refequipo 	= ".$refequipo.",
	reffixture 	= ".$reffixture.",
	amarillas 	= ".($amarillas == '' ? 'null': $amarillas).",
	azul 		= ".($azul == '' ? 'null': $azul).",
	rojas 		= ".($rojas == '' ? 'null': $rojas).",
	jugo 		= '".($jugo == '' ? 'null': ($jugo == '1' ? true : false))."',
	cancha 		= ".($cancha == '' ? 'null': $cancha).",
	arquero 	= ".($arquero == '' ? 'null': $arquero).",
	puntos 		= ".($puntos == '' ? 'null': $puntos).",
	mejor 		= '".($mejor == '' ? 'null': ($mejor == '1' ? true : false))."',
